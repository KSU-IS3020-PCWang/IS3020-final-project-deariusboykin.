...
"""Stage 1: Store reservations in a list while the program is running."""


def add_reservation(
    reservations, customer_name, phone, date, time, party_size
):
    """Create one reservation dictionary and add it to the list."""
    reservation = {
...
"""Stage 2: Add a function that displays every reservation."""
...
def display_reservations(reservations):
    """Print all reservations in a readable format."""
...
"""Stage 3: Save the in-memory reservation list to a CSV file."""
...
if save_reservations(DATA_FILE, reservations):
    print("The reservation was saved to the CSV file.")

display_reservations(reservations)

"""Stage 4: Load saved reservations when the program starts."""
...
"""Stage 5: Find a reservation by ID and modify its information."""
...
"""Stage 6: Delete a reservation and save the changed list."""
...
reservation_to_delete = reservations[len(reservations) - 1]
delete_reservation(reservations, reservation_to_delete["id"])
save_reservations(DATA_FILE, reservations)
display_reservations(reservations)

"""Stage 7: Check time slots and display customer reservation history."""
...
def add_reservation(
    ...
    """Add a reservation if the requested time slot is available."""
...
def modify_reservation(
    ...
    """Modify a reservation if its new time slot is available."""
    reservation = find_reservation(reservations, reservation_id)
...
conflicting_reservation = add_reservation(
    reservations,
    "Marcus Green",
    "404-555-4567",
    "2026-07-20",
...
if conflicting_reservation is None:
    print("The occupied time slot was correctly rejected.")

customer_history = get_customer_history(reservations, "404-123-1234")
